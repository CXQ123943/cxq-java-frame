package com.steven.mapper;

import com.steven.pojo.Dept;
import com.steven.pojo.Emp;
import com.steven.pojo.Student;
import org.apache.ibatis.annotations.One;
import org.apache.ibatis.annotations.Result;
import org.apache.ibatis.annotations.Results;
import org.apache.ibatis.annotations.Select;
import org.apache.ibatis.type.JdbcType;

import java.util.List;

/**
 * @author StevenChen
 * @version 1.0
 */
public interface EmpMapper {

    /**
     * 查询所有员工，同时查出员工所在部门信息
     *
     * @return 所有员工及其部门信息
     * */
    List<Emp> findEmpWithDeptByJoin();

    /**
     * 查询所有员工，同时查出员工所在部门信息
     *
     * @return 所有员工及其部门信息
     * */
    List<Emp> findEmpWithDeptBySelect();

    /**
     * 查询所有员工，同时查出员工所在部门信息
     *
     * @return 所有员工及其部门信息
     */
    @Select("SELECT `empno`, `ename`, `job`, `mgr`, `hiredate`, `sal`, `comm`, `deptno` FROM mybatis.`emp`")
    @Results(id = "emp-dept-one", value = {
            @Result(column = "empno", property = "empno", jdbcType = JdbcType.INTEGER, id = true),
            @Result(column = "deptno", property = "dept", javaType = Dept.class,
                    one = @One(select = "com.steven.mapper.DeptMapper.findByDeptNo"))
    })
    List<Emp> findWithDeptByOne();
}
